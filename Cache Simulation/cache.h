...
class cache{
    ...
    public:
        ...
        cache(int bytes = 0, int associability = 0 , int block = 0, int stream_count = 0, int stream_size = 0, cache *next = nullptr){
        ...
        void request(char rw, uint64_t addy){   //function that handles read/write requests
            ...
            index = (addy / sets) & mask;                    //index bits
                    ...
                    if(stream_buffers[i].valid){                    //if stream buffer is valid
                        for(int j = 0; j < stream_buffers[i].tags.size(); j++){ //loop through all tags in stream buffer
                            if(stream_buffers[i].tags[j] == tag){   //if tags match
                                ...
                                cache_store(set_list[index], tag);          //store new block in cache
                                if(rw == 'w'){                       //if write request
                                    writes++;                        //increment write counter
...
void block_adjust(vector<cache::block> &v, int index){
...
void stream_adjust(vector<cache::stream> &v, int index){
    for(int i = 0; i < v.size(); i++){
        if(v[i].valid){
            v[i].recency++;                 //increase the recency of all blocks older than the accessed block
        }
...
void stream_store(vector<cache::stream> &v, uint64_t tag, int stream){
...
void cache_store(vector<cache::block> &v, uint64_t tag){          //set(not set_list) is passed as argument
